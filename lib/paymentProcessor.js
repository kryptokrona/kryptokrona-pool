    ...
    function (keys, callback) {
      ...
      global.redisClient.multi(redisCommands).exec(function (error, replies) {
        ...
        for (var i = 0; i < replies.length; i++) {
          ...
          var minerAddress
          var minPayment = global.config.payments.minPayment
          ...
          try {
            ...
            if (minerAddress.paymentId.length !== 0) {
              minPayment = global.config.payments.minPaymentIdPayment
          ...
          minPayoutLevel[workerId] = parseFloat(data[1]) || minPayment
    ...
    function (balances, minPayoutLevel, callback) {
      ...
      for (worker in payments) {
        ...
        try {
          ...
          global.log('error', logSystem, 'Skipping payment to invalid miner payment address %s', [workerId])
          continue
        }
