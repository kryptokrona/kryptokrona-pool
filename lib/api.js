...
function handleSetMinerPayoutLevel(urlParts, response){
    ...
    if (level < currentStats.config.minPaymentThreshold/100) {
        response.end(JSON.stringify({'status': 'Please choose a value above ' + currentStats.config.minPaymentThreshold/100}));
        return;
    }
    ...
    minerSeenWithIPForAddress(address, ip, function (error, keys) {
        ...
        redisClient.hset(config.coin + ':workers:' + address, 'minPayoutLevel', payout_level, function(error, value){
            ...
            log('info', logSystem, 'Updated payout level for address ' + address + ' level: ' + payout_level);
...
var server = http.createServer(function(request, response){
    ...
    switch(urlParts.pathname){
        ...
        case '/set_miner_payout_level':
            urlParts.query.ip = getCallerIP(request);
