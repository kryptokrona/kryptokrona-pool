...
function getUserHashrateChartData(address, workers, callback) {
    for(var worker in workers) {
        getChartDataFromRedis('hashrate:' + address + '+' + worker, callback);
...
function getUserChartsData(address, workers, paymentsData, callback) {
    var stats = {};
    var chartsFuncs = {
...
function collectUsersHashrate(chartName, settings) {
    ...
    redisClient.keys(redisBaseKey + '*', function(keys) {// turtlecoin:charts:hashrate:*
        var hashrates = {};
        for(var i in keys) {
            //log('info', logSystem, keys[i]);
            hashrates[keys[i].substr(keys[i].length)] = 0;
        }
        getUsersHashrates(function(newHashrates) {
            for(var address in newHashrates) {
                var nameOffset = address.indexOf('+');
                if (nameOffset != -1) {
                    var AddressParts = address.split('+');
                    hashrates[AddressParts[0]] = parseInt(newHashrates[address]) + (hashrates[AddressParts[0]] || 0);
                }
                else {
