...
var shareTrustProbabilityStepPercent = shareTrustEnabled ? config.poolServer.shareTrust.stepDown / 100 : 0
...
Miner.prototype = {
  ...
  checkTrust: function (difficulty) {
    ...
    if ((difficulty < config.poolServer.shareTrust.maxTrustedDifficulty) && (dateNowSeconds - shareTrustIP.trustBeginSeconds > config.poolServer.shareTrust.minUntrustedSeconds && shareTrustIP.threshold <= 0 && Math.random() < shareTrustIP.probability) && (dateNowSeconds - shareTrustAddress.trustBeginSeconds > config.poolServer.shareTrust.minUntrustedSeconds && shareTrustAddress.threshold <= 0 && Math.random() < shareTrustAddress.probability)) {
      return true;
    }
    return false;
  },
...
function processShare (miner, job, blockTemplate, nonce, resultHash) {
  ...
  if (shareTrustEnabled && shareType == 'valid') { miner.setTrust(true) }
