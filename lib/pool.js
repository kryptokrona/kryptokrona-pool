...
Miner.prototype = {
  ...
  checkTrust: function (difficulty) {
    ...
    if (ipPastThresholds && addressPastThresholds && shareTrustIP.probability > shareTrustMinFloat && shareTrustAddress.probability > shareTrustMinFloat){
      this.trusted = true
    }
...
function processShare (miner, job, blockTemplate, nonce, resultHash) {
  ...
  if (shareTrustEnabled) {
    ...
    if (miner.trusted && minerTrusted == false) { log('info', logSystem, 'Miner is now share trusted %s@%s', [miner.login, miner.ip]) }
  }
