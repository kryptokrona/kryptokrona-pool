...
var shareTrustProbabilityStepPercent = shareTrustEnabled ? config.poolServer.shareTrust.probabilityStepPercent / 100 : 0
var shareTrustMaxTrustPercent = shareTrustEnabled ? config.poolServer.shareTrust.maxTrustPercent / 100 : 0
...
Miner.prototype = {
  ...
  checkTrust: function (difficulty) {
    ...
    var ipPastThresholds = dateNowSeconds - shareTrustIP.trustBeginSeconds > config.poolServer.shareTrust.minUntrustedSeconds && shareTrustIP.threshold <= 0
    var addressPastThresholds = dateNowSeconds - shareTrustAddress.trustBeginSeconds > config.poolServer.shareTrust.minUntrustedSeconds && shareTrustAddress.threshold <= 0
    if (ipPastThresholds && addressPastThresholds && shareTrustIP.probability >= shareTrustMaxTrustPercent && shareTrustAddress.probability >= shareTrustMaxTrustPercent){
      this.trusted = true
    }
    if ((difficulty < config.poolServer.shareTrust.maxTrustedDifficulty) && (ipPastThresholds && Math.random() < shareTrustIP.probability) && (addressPastThresholds && Math.random() < shareTrustAddress.probability)) {
      return true;
    }
    return false;
  },
  setTrust: function (shareValidated) {
    ...
    if (shareTrustIP && shareTrustAddress) {
      if (shareValidated) {
        shareTrustIP.probability += shareTrustProbabilityStepPercent
        if (shareTrustIP.probability > shareTrustMaxTrustPercent) { shareTrustIP.probability = shareTrustMaxTrustPercent }
        if (shareTrustIP.threshold > 0) { shareTrustIP.threshold-- }
        shareTrustAddress.probability += shareTrustProbabilityStepPercent
        if (shareTrustAddress.probability > shareTrustMinFloat) { shareTrustAddress.probability = shareTrustMaxTrustPercent }
        if (shareTrustAddress.threshold > 0) { shareTrustAddress.threshold-- }
      } else {
        shareTrustIP.probability = shareTrustAddress.probability = 0
        shareTrustAddress.threshold = shareTrustIP.threshold = (config.poolServer.shareTrust.threshold * config.poolServer.shareTrust.penaltyMultiplier)
...
function processShare (miner, job, blockTemplate, nonce, resultHash) {
  ...
  var minerTrusted = miner.trusted

  if (shareTrustEnabled && miner.checkTrust(job.difficulty)) {
  ...
  if (hash.toString('hex') !== resultHash) {
    ...
    if (shareTrustEnabled) {
      miner.setTrust(false)
      if (minerTrusted) { log('warn', logSystem, 'Share trust broken by %s@%s', [miner.login, miner.ip]) }
    }
    return false
  }
  ...
  if (shareTrustEnabled) {
    if (shareType == 'valid') { miner.setTrust(true) }
    if (miner.trusted && !minerTrusted) { log('info', logSystem, 'Miner is now share trusted %s@%s', [miner.login, miner.ip]) }
  }
