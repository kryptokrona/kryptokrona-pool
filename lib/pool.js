...
const acceptableAddressLength = global.config.poolServer.poolAddress.length
...
function handleMinerMethod (method, params, ip, portData, sendReply, pushMessage) {
  /* Check for malformed miner parameters */
  if (typeof params !== 'object' ||
    typeof params.id === 'undefined' ||
    params.id.length === 0 ||
    typeof params.login === 'undefined' ||
    params.login.length === 0 ||
    typeof params.pass === 'undefined' ||
    params.pass.length === 0) {
    /* If the miner sent malformed parameters, ban them by IP */
    ...
    case 'login':
      ...
      if (workerNameCharPos !== -1) {
        ...
        if (global.config.poolServer.fixedDiff.enabled) {
          fixedDiffCharPos = login.indexOf(global.config.poolServer.fixedDiff.addressSeparator)
          workerName = workerName.split(global.config.poolServer.fixedDiff.addressSeparator)[0]
        }
      ...
      if (acceptableAddressLength !== login.length) {
        sendReply('invalid address length used for login')
        return
      }
